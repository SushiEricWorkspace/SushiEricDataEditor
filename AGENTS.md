...
- force pushは明示的に依頼されない限り使用しない。`ai/work`の同期にforce pushが必要な場合も、ユーザーの明示的な許可なしに実行しない。
...
- ユーザー管理のローカル環境では、通常の開発作業は最新の`develop`を基準に行い、commitやpushの許可がある場合は現在の開発ブランチへ反映する。
- AI側の独立した作業環境からGitHubへ直接変更する場合は、上記のブランチ運用に従って`ai/work`へpushし、開発基準ブランチ宛てのPRを作成する。
